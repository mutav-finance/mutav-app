...
export const OUT_OF_SCOPE_TABLES = {
  ...
  products:
    "The guarantee product catalog — pricing parameters (rate table, multipliers, fees, commission), an eligibility rule and an effective window. Every column describes what Mutav sells, not to whom; the per-person copy of these numbers is the `guarantees.terms` snapshot, which is classified. `eligibility.agencyIds` names agencies (legal entities, already covered on `agencies`), never a natural person.",
} as const;
...
export const PII_FIELD_REGISTRY = {
  ...
  users: {
    ...
    name: {
      ...
      subjectType: SUBJECT.PLATFORM_USER,
      legalNormRef: null,
      note: "Copied into `guaranteeHistory.username`.",
    },
    email: {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
  ...
  contractApplications: {
    ...
    cep: {
      ...
      note: "Not in §4.2. Classified by analogy with `leases.property.cep` (T2). §4.3 names CEP a race/class proxy, so it is also an input the sensitive-inference guard (E-13) must see.",
    },
    rentCents: {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Not in §4.2. Classified by analogy with `leases.rent.*Cents` (T2).",
    },
    openedBy: {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
  ...
  // The rental relationship. Carries the property (the tenant's home address)
  // and the living rent bundle; the guarantee sold against it is a separate
  // table below.
  leases: {
    agencyId: {
      ...COUNSEL_PENDING,
    ...
    openGuaranteeId: {
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Not in §4.2. Pointer to the one non-closed guarantee on this lease.",
    },
  },

  // What Mutav sells against a lease: lifecycle state, the underwriting
  // snapshot, the immutable pricing terms and the remaining capacity.
  guarantees: {
    agencyId: {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
    ...
    leaseId: {
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: "LGPD art. 13 §4",
      note: "The join to the lease and, through it, to the tenant.",
    },
    publicId: {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
    ...
    productId: {
      ...
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Not in §4.2. Catalog reference; `products` holds no personal data.",
    },
    status: {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
    ...
    "closure.reason": {
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Not in §4.2. Why the guarantee ended (`eviction`, `abandonment`, `death`, …) is an outcome about the tenant's tenure. Classified by analogy with `tenantApproval.status` (T2), not with the notice `status` (T1) — the negative credit-behaviour record itself lives on `guaranteeDelinquencyNotices`.",
    },
    "closure.closedAt": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
    ...
    "underwriting.tier": {
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: "LGPD art. 12 §2; Sumula 550/STJ",
      note: "Not in §4.2. The band the score falls into — classified with the score it derives from, as `creditAnalysisAssessments.tier` already is.",
    },
    "underwriting.assessmentId": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
      dataClass: CLASS.OPERATIONAL_REFERENCE,
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Not in §4.2. Reference to the assessment row the score was read from.",
    },
    ...
    "terms.productSlug": {
      ...
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Not in §4.2. Which catalog product priced this guarantee.",
    },
    "terms.plan": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
    ...
    "terms.rentCents": {
      ...
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Rent at pricing time; `leases.rent.rentCents` is the living value.",
    },
    "terms.feeCents": {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
    ...
    "terms.oneTimeActivationFeeCents": {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
      dataClass: CLASS.FINANCIAL_OBLIGATION,
      subjectType: SUBJECT.TENANT,
    ...
    "terms.coverageCeilingMultiplier": {
      ...
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Not in §4.2. Pricing parameter copied from the product; says nothing about the person.",
    },
    "terms.exitCostMultiplier": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
      dataClass: CLASS.OPERATIONAL_REFERENCE,
      subjectType: SUBJECT.NONE,
    ...
    "capacity.reservedCents": {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
      dataClass: CLASS.FINANCIAL_OBLIGATION,
      subjectType: SUBJECT.TENANT,
      legalNormRef: null,
      note: "Capacity earmarked by committed cover — a mirror of the arrears the reserve is standing behind.",
    },
    "documents[].key": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
  ...
  guaranteeHistory: {
    ...
    "transition.from": {
      ...
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Lifecycle state the guarantee left. Describes the product, not a person.",
    },
    "transition.to": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
      ...
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Lifecycle state the guarantee entered.",
    },
    "transition.closeReason": {
      ...COUNSEL_PENDING,
      tier: TIER.T3,
      dataClass: CLASS.OPERATIONAL_REFERENCE,
      subjectType: SUBJECT.NONE,
      legalNormRef: null,
      note: "Closed-value-object reason; a fixed enum, never free text.",
    },
  ...
  guaranteeDelinquencyNotices: {
    ...
    status: {
      ...
      note: "§4.2 puts `status` in the T1 group with the amounts, not with the other status columns: joined to `tenants` via `guaranteeId` it is a negative credit-behaviour record about a named natural person.",
    },
    rentDueDate: {
      ...COUNSEL_PENDING,
      tier: TIER.T1,
    ...
    "resolution.appliedCoverCents": {
      ...COUNSEL_PENDING,
      tier: TIER.T1,
      dataClass: CLASS.CREDIT_RISK,
      subjectType: SUBJECT.TENANT,
      legalNormRef: "CDC art. 43 §1",
      note: "Not in §4.2. The reserve actually drawn against the guarantee for this default — bounded by, and disclosing, the arrears amount. Same masking rule as `updatedAmountCents`.",
    },
    "resolution.note": {
      ...COUNSEL_PENDING,
      tier: TIER.T2,
