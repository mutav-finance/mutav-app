...
export class EtherfuseClient implements Anchor {
  ...
  async getFiatAccounts(customerId: string): Promise<SavedFiatAccount[]> {
    try {
      ...
      return response.items.map((account) => {
        ...
        const isPix = currency === "brl" || (currency === undefined && !!account.pixKey);
        return {
   ...
   * @throws {AnchorError} If `publicKey` is missing or on API failure.
   */
  async getKycUrl(customerId: string, publicKey?: string, bankAccountId?: string): Promise<string> {
    if (!publicKey) {
      throw new AnchorError("publicKey is required for KYC onboarding", "MISSING_PUBLIC_KEY", 400);
    }
  ...
  async acceptCustomerAgreement(presignedUrl: string): Promise<EtherfuseAgreementResponse> {
    return this.request<EtherfuseAgreementResponse>("POST", "/ramp/agreements/customer-agreement", {
      presignedUrl,
    });
  }
