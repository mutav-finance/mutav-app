...
| Logged in, but **no guarantees**                                                          | A partial seed (only the demo agencies) — you're on a login persona's own agency, which that path skips. Run `bun run seed` (`seedReset`).                                |
