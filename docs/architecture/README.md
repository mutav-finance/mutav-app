...
- [`underwriting.md`](underwriting.md) — Tenant underwriting pipeline (verify → credit analysis → offer products); the `compliance` (verification) / `creditAnalysis` (credit analysis) / `products` (catalog) / `guarantees` (coverage decision) composition.

**Lifecycle spec:**

- [`guarantee-lifecycle.md`](guarantee-lifecycle.md) — the seven guarantee states, the transition matrix, close reasons, the notice ↔ guarantee coupling, capacity policy C, the single write path, and the audit trail — every rule cited to the line that enforces it and the test that proves it, plus the deviations from the protocol scenarios document. Read this before touching `convex/guarantees/` or `convex/delinquencies/`.
...
| **Tenant**         | `(public)/pay/[publicId]/*`               | None — unguessable `publicId` in URL acts as bearer token                                                                                                                        | Untrusted by default; gated by `publicId` knowledge and invoice status                                                   |
...
| `pay/[publicId]/*`            | `apps/pay`    | `<FlowShell>` | `publicId` bearer + invoice status                   | **always empty**     | Built — deploy pending                                                                                    |
...
| `leases`        | Shipped     | The rental relationship — property, rent, payer, tenant pointer, `openGuaranteeId`. One lease takes many guarantees over time, at most one of them non-`closed`.                                                                                              | [`decisions/0007-lease-guarantee-split.md`](decisions/0007-lease-guarantee-split.md) |
| `guarantees`    | Shipped     | What Mutav sells against a lease — 7-state lifecycle (`drafted · active · in_arrears · default_verified · cover_committed · in_eviction · closed`), immutable `terms` snapshot, coverage capacity, documents, history. Renamed from `contracts`.              | [`decisions/0007-lease-guarantee-split.md`](decisions/0007-lease-guarantee-split.md) |
| `products`      | Shipped     | Pricing parameters as data — `terms` bundle, eligibility rules, effective window. Editing a product never touches a sold guarantee; the guarantee reads its own snapshot.                                                                                     | [`underwriting.md`](underwriting.md)                                                 |
...
| `screening`     | Shipped     | Vendor-neutral external-data signals (credit, registration, sanctions) + reproducible assessment snapshots. Consumed by `guarantees` (tenant underwriting) and, in Phase 2, `compliance` (agency KYB).                                                        | [`../../convex/screening/README.md`](../../convex/screening/README.md)               |
...
| `delinquencies` | **Live**    | Delinquency notices (`open → verified → resolved / canceled`) that drive the guarantee's default states in the same transaction; agency reporting, staff verify / cover / dismiss; the admin `/defaults` queue                                                | [`guarantee-lifecycle.md`](guarantee-lifecycle.md) § 6                               |
...
7. **`publicId` ↔ Tenant access** — bearer token boundary. The tenant pays via a URL containing an unguessable id. The id is not rotated today; revocation is "void the invoice".
