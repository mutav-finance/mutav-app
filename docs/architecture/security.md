...
| `AUDIT_ANCHOR_SECRET` (optional)      | Stellar secret seed for the dedicated audit-anchor account (single-sig)   | Node    | [`convex/lib/env.ts`](../../convex/lib/env.ts) → `getAuditAnchorSecret()`          | `convex/audit/actions.ts` → `submitDailyAnchor`. Null in dev/preview.      |
...
### External witness (P1b landed)

Without an external anchor, `verifyChain` alone wouldn't catch a full-chain rewrite by an attacker with Convex admin access (recompute every entry's hash under tampered values → chain passes verification). **P1b closes this by daily-anchoring a Merkle root of the chain to Stellar**, where rewriting history would require rewriting Stellar itself — the actual security boundary.

The daily flow (cron at 00:10 UTC):

1. `computeAuditAnchor` reads every `mutavAuditLog.entryHash` since the last anchor's `periodEnd`, computes a Merkle root, inserts a `pending` row in `mutavAuditAnchors`.
2. `submitDailyAnchor` (action) submits a Stellar transaction from a dedicated single-sig account (`AUDIT_ANCHOR_SECRET`) carrying the root as `MEMO_HASH` plus a `manageData` operation keyed by the period (`audit.YYYY-MM-DD`). On success, the anchor row flips to `submitted` with the Stellar `txHash` and `network`.
3. `verifyLatestAnchor` query recomputes the root from the audit entries in the latest anchor's period and compares — mismatch surfaces as `root_mismatch` with the broken anchor's id.

The Merkle construction is deterministic (carry-up on odd levels, separator `|` between paired hashes) so any reader with the audit entries plus the on-chain memo can independently verify the chain didn't change between commit and read.

**Dev/preview mode** (no `AUDIT_ANCHOR_SECRET`): the cron still runs and persists `pending` rows with valid root computations. `verifyLatestAnchor` works against local data — useful for catching mid-test tampering — but without the Stellar witness, an attacker with full Convex access can still rewrite the entire chain _and_ the anchor table. Real cross-system integrity arrives the moment production sets the env var and the first real anchor lands on chain. This is the deliberate dev-only stance documented in the [key management guide](../key-management-guide.md): same code paths, same primitives, only the external witness comes online at launch.

**Anchor account compromise** is bounded: an attacker who steals `AUDIT_ANCHOR_SECRET` could submit a wrong root, and `verifyLatestAnchor` would surface the mismatch on the next verification cycle. No funds at risk — the account holds only enough XLM (~10) to fund daily 100-stroop memo transactions for years. Rotate via `bunx convex env set AUDIT_ANCHOR_SECRET S...` with a fresh account; old anchors remain valid because each one is independently verifiable against its own period.

**Runbook commands** (no admin UI yet — that lands with the `(admin)` shell):

```bash
# Compute + submit today's anchor on demand
bunx convex run audit/actions:submitDailyAnchor

# Verify the latest anchor's root matches the current audit entries
bunx convex run audit/anchor:verifyLatestAnchor

# Inspect the most recent N anchors (Convex dashboard query)
bunx convex run "anchors:list" --limit 10
```
...
| **Audit log integrity**      | **P1a + P1b landed.** Hash-chained `mutavAuditLog` wired into every money-moving mutation; daily Merkle root anchored to Stellar via dedicated `AUDIT_ANCHOR_SECRET` account. `verifyLatestAnchor` checks the chain against the on-chain witness. P1c retention policy still open. | LGPD P1 ([#79](https://github.com/mutav-finance/mutav-app/issues/79)) — complete. Retention policy tracked separately in [#114](https://github.com/mutav-finance/mutav-app/issues/114) → P1c. |
